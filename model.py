...
def Conv_Block(x, filter_size, bottleneck=False, dropout_rate=None, weight_decay=1e-4):
...
class PDPAttention(Layer):
    ...
    def build(self, input_shape):
        ...
        self.b_e = self.add_weight(name='b_e',
                                   ...
                                   initializer='uniform',
                                   trainable=True)
        super(PDPAttention, self).build(input_shape)
...
def Time_Freq_3DCNN(x, input_shape):
    concat_axis = 1 if K.image_data_format() == 'channels_first' else -1
...
def Model_3DCNN_3stream(nb_channels, nb_freq_band, temp_length, spe_length):
    ...
    x_ts = Time_Freq_3DCNN(speInput,(nb_channels, nb_freq_band, spe_length,1))
    tsInput = Input([nb_channels, nb_freq_band, 1])
    x_s = Freq_2DCNN(tsInput,(nb_channels, nb_freq_band,1))
